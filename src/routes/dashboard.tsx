...
async function waitWithHeartbeat(ms: number, jobId: string) {
  const HEARTBEAT_INTERVAL = 20_000; // send a heartbeat every 20s during long waits
  const steps = Math.ceil(ms / HEARTBEAT_INTERVAL);
  const stepMs = Math.ceil(ms / Math.max(1, steps));
  for (let i = 0; i < steps; i++) {
    await wait(Math.min(stepMs, ms - i * stepMs));
    if (i < steps - 1) {
      // keep the heartbeat alive so the frontend doesn't think the job is dead
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      if ([429, 502, 503, 504, 524].includes(status)) {
        ...
        await waitWithHeartbeat((status === 429 ? 20_000 : 3_000) * attempt, jobId);
        continue;
      }
      ...
      if (aborted) await waitWithHeartbeat(2_000 * attempt, jobId);
      else await waitWithHeartbeat(1_500 * attempt, jobId);
    }
  }
  ...
  for (let start = 0; start < chunks.length; start += strategy.parallelLimit) {
    ...
    const results = await Promise.all(
      batch.map((chunk, index) => {
        ...
        return runWithRetry(TEXT_MODEL, [{ role: "user", content: prompt }], jobId);
      }),
    );
...
async function extractFromImage(
  ...
  const result = await runWithRetry(VISION_MODEL, messages, jobId);

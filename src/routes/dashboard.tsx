...
function Dashboard() {
  ...
  // ── Per-file parallel tracking ───────────────────────────────────────────
  const [fileStatuses, setFileStatuses] = useState<
    Record<string, { progress: number; currentStage: string | null; done: boolean; error: string | null }>
  >({});
  ...
  const runJob = async (
    ...
    while (Date.now() < deadline) {
      ...
      if (onProgress) {
        // Use real progress if available, otherwise estimate based on elapsed time (cap at 90%)
        const realProgress = s.progress ?? 0;
        const elapsed = Date.now() - jobStart;
        const estimatedProgress = Math.min(90, 20 + Math.floor(elapsed / 1000) * 1.5);
        const displayProgress = realProgress > 20 ? realProgress : estimatedProgress;
        const stage = s.currentStage
          ? s.totalChunks > 1
            ? `${s.currentStage} (${s.processedChunks}/${s.totalChunks})`
            : s.currentStage
          : s.status === "processing"
            ? `Processing… (${Math.floor((Date.now() - jobStart) / 1000)}s)`
            : "Waiting for worker…";
        onProgress(displayProgress, stage);
      }
    ...
    return {
      ok: false,
      error: "This document is taking too long to process in the background. Please retry with a smaller file or scan fewer pages.",
    };
  };
  ...
  // ── Extract a single file (parallel-safe: no global scanning state) ──────
  const runExtraction = async (file: File): Promise<ExtractedRow[]> => {
    if (!email) return [];

    const fileKey = `${file.name}-${file.size}`;
    const started = Date.now();

    const updateStatus = (progress: number, currentStage: string | null, error: string | null = null, done = false) => {
      setFileStatuses((prev) => ({ ...prev, [fileKey]: { progress, currentStage, done, error } }));
    };

    updateStatus(3, "Preparing upload");
    ...
    try {
      ...
      if (isImage) {
        updateStatus(8, "Preparing image");
        try {
          ...
          res = await runJob({ kind: "image", imageDataUrl, fileName: file.name }, (p, s) => updateStatus(p, s));
        } catch (e) {
        ...
        updateStatus(10, "Reading PDF pages");
        ...
        if (charCount < 20) {
          updateStatus(15, "Scanned PDF — switching to vision…");
          ...
          if (charCount > WARN_CHAR_LIMIT) {
            toast.warning(`Large document: ${file.name} — processing in background…`);
          }
          updateStatus(20, "Sending to AI…");
          res = await runJob({ kind: "text", text: extracted.text, fileName: file.name }, (p, s) => updateStatus(p, s));

          if (!res.ok && res.error === "__NEEDS_VISION__") {
            updateStatus(25, "Scanned PDF — switching to vision…");
            const imageDataUrl = await pdfPageToImageDataUrl(file, 1);
            res = await runJob({ kind: "image", imageDataUrl, fileName: file.name }, (p, s) => updateStatus(p, s));
          }
        }
      ...
      if (!res.ok) {
        ...
        toast.error(`Failed: ${file.name}`, {
          ...
          action: { label: "Retry", onClick: () => onFiles([file]) },
        });
        updateStatus(0, "Failed", res.error, true);
        return [];
      }

      const extractedRows: ExtractedRow[] = res.rows.map((r) => ({
      ...
      if (!recorded.ok) {
        ...
        updateStatus(0, "Failed", recorded.error, true);
        return [];
      }
      setUsage(recorded.usage);
      ...
      toast.success(`✓ ${file.name}`, {
        description: `Processed in ${((Date.now() - started) / 1000).toFixed(1)}s`,
      });
      updateStatus(100, "Completed", null, true);
      ...
      updateStatus(0, "Failed", error, true);
      return [];
    }
  };
  ...
  const onFiles = async (files: File[]) => {
    ...
    if (!usage.unlimited && usage.remaining < files.length) {
      toast.error("Usage limit reached", {
        description: `You need ${files.length} uploads but only have ${usage.remaining} remaining.`,
        action: { label: "Upgrade", onClick: () => setUpgrade(true) },
      });
      return;
    }

    // Show the first file in the PDF preview
    setCurrentFile(files[0]);
    setLastFile(files[files.length - 1]);
    setRows([]);
    setScanning(true);

    // Reset per-file statuses
    const initialStatuses: Record<string, { progress: number; currentStage: string | null; done: boolean; error: string | null }> = {};
    for (const f of files) {
      initialStatuses[`${f.name}-${f.size}`] = { progress: 0, currentStage: "Queued", done: false, error: null };
    }
    setFileStatuses(initialStatuses);

    toast.info(`Processing ${files.length} file${files.length === 1 ? "" : "s"} in parallel…`);

    try {
      // Run all files simultaneously
      const results = await Promise.all(files.map((f) => runExtraction(f)));
      const allRows = results.flat();

      if (allRows.length > 0) {
        setRows(allRows);
        setHistory((prev) => {
          const updated = [...allRows, ...prev];
          saveHistory(updated);
          return updated;
        });
        toast.success(`All done! Extracted ${allRows.length} record${allRows.length === 1 ? "" : "s"} from ${files.length} file${files.length === 1 ? "" : "s"}.`);
      }
    } finally {
      setScanning(false);
    }
                    ...
                    {scanning ? (
                      <div className="glass rounded-2xl p-4 space-y-3">
                        <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">
                          Processing {Object.keys(fileStatuses).length} file{Object.keys(fileStatuses).length === 1 ? "" : "s"} in parallel…
                        </p>
                        {Object.entries(fileStatuses).map(([key, s]) => {
                          const fileName = key.replace(/-\d+$/, "");
                          return (
                            <div key={key} className="space-y-1">
                              <div className="flex items-center justify-between text-xs">
                                <span className="truncate text-foreground max-w-[70%]">{fileName}</span>
                                <span className={`text-xs ${s.error ? "text-destructive" : s.done ? "text-lime" : "text-muted-foreground"}`}>
                                  {s.error ? "Failed" : s.done ? "Done ✓" : s.currentStage ?? "Queued"}
                                </span>
                              </div>
                              <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                                <div
                                  className={`h-full transition-all duration-500 ${s.error ? "bg-destructive" : s.done ? "bg-lime" : "bg-lime/60"}`}

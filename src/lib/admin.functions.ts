...
import { eq, gte, count, and, desc, sql } from "drizzle-orm";
...
export const getAdminStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [allProfiles, uploads24hRows, uploads7dRows, tickets, subs] = await Promise.all([
      ...
      db.select({ userId: uploads.userId, createdAt: uploads.createdAt })
        .from(uploads).where(gte(uploads.createdAt, since7d)),
    ...
    return {
      ...
      uploadsToday: uploads24hRows[0]?.count ?? 0,
      uploadsByDay: days,
      topUsers,
...
export const getAdminUsers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<AdminUser[]> => {
    ...
    for (const s of subsRows) subByUser[s.userId] = { plan: s.plan, status: s.status };

    const uploads24hCount: Record<string, number> = {};
    for (const u of uploadsRows) uploads24hCount[u.userId] = (uploads24hCount[u.userId] ?? 0) + 1;
